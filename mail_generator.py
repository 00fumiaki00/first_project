...
while True:
    instruction = input("指示書の内容を入力してください（終了する場合は「終了」と入力）: \n")
    if instruction == "終了":
        break
...
"""
    ...
    if "repeat_keyword" in selected:
    ...
    elif "common_file" in selected:
        with open(selected["common_file"], "r", encoding="utf-8") as f:
            common = f.read()
        ...
        for branch in selected["branches"]:
            if any(keyword in instruction for keyword in branch.get("conditions",[])):
                with open(branch["content_file"], "r", encoding="utf-8") as f:
                    content = f.read()
                prompt = common + content + f"\n\n【指示書】\n{instruction}"
                matched = True      # 「見つかった」に書き換える
                break
        if not matched:
            default_branch = next(b for b in selected["branches"] if b.get("default"))
            with open(default_branch["content_file"], "r", encoding="utf-8") as f:
                content = f.read()
            prompt = common + content + f"\n\n【指示書】\n{instruction}"
        ...
        with open(selected["prompt_file"], "r", encoding="utf-8") as f:
